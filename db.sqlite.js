...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS app_executions (
    ...
    error_message TEXT,
    created_at INTEGER NOT NULL,
    FOREIGN KEY(app_id) REFERENCES flow_apps(id)
  );
...
export function getApp(id) {
  const stmt = db.prepare("SELECT * FROM flow_apps WHERE id = ?");
...
export function getAppBySlug(slug) {
  const stmt = db.prepare("SELECT * FROM flow_apps WHERE slug = ?");
...
export function listApps({ status = "published", limit = 50, offset = 0, category = null } = {}) {
  let query = "SELECT * FROM flow_apps WHERE status = ?";
  const params = [status];
  if (category) {
    query += " AND category = ?";
...
export function updateApp(id, data) {
  ...
  const query = `UPDATE flow_apps SET ${updates.join(", ")} WHERE id = ?`;
  const stmt = db.prepare(query);
  stmt.run(...params);
...
export function deleteApp(id) {
  const stmt = db.prepare("DELETE FROM flow_apps WHERE id = ?");
  stmt.run(id);
}

export function recordExecution(app_id, { user_id = null, input_data = null, output_data = null, status = "success", execution_time_ms = 0, error_message = null } = {}) {
  const id = crypto.randomBytes(16).toString("hex");
  const now = Math.floor(Date.now() / 1000);
  ...
  const updateStmt = db.prepare("UPDATE flow_apps SET usage_count = usage_count + 1 WHERE id = ?");
  updateStmt.run(app_id);
}

export function getExecutions(app_id, { limit = 20 } = {}) {
  const stmt = db.prepare(
    "SELECT * FROM app_executions WHERE app_id = ? ORDER BY created_at DESC LIMIT ?"
  );
  return stmt.all(app_id, limit).map((row) => ({
    ...row,
...
export function addReview(app_id, user_id, rating, review_text = "") {
  const id = crypto.randomBytes(16).toString("hex");
  const now = Math.floor(Date.now() / 1000);
  ...
  stmt.run(id, app_id, user_id, rating, review_text, now);

  // Update app rating
  const avgStmt = db.prepare("SELECT AVG(rating) as avg_rating FROM app_reviews WHERE app_id = ?");
  const result = avgStmt.get(app_id);
  const avgRating = result?.avg_rating || 0;

  const updateStmt = db.prepare("UPDATE flow_apps SET rating = ? WHERE id = ?");
  updateStmt.run(avgRating, app_id);
}

export function getReviews(app_id) {
  const stmt = db.prepare("SELECT * FROM app_reviews WHERE app_id = ? ORDER BY created_at DESC");
  return stmt.all(app_id);
}

export function addFavorite(user_id, app_id) {
  const now = Math.floor(Date.now() / 1000);
  const stmt = db.prepare(`
    INSERT OR IGNORE INTO user_app_favorites (user_id, app_id, created_at)
    VALUES (?, ?, ?)
  `);
  stmt.run(user_id, app_id, now);
}

export function removeFavorite(user_id, app_id) {
  const stmt = db.prepare("DELETE FROM user_app_favorites WHERE user_id = ? AND app_id = ?");
  stmt.run(user_id, app_id);
}
