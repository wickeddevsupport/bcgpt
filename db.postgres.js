...
async function ensureSchema() {
  ...
  await pool.query(`
    ...
    CREATE INDEX IF NOT EXISTS idx_flow_apps_slug ON flow_apps(slug);
    CREATE INDEX IF NOT EXISTS idx_flow_apps_status ON flow_apps(status);
    CREATE INDEX IF NOT EXISTS idx_flow_apps_created_by ON flow_apps(created_by);
    CREATE INDEX IF NOT EXISTS idx_flow_apps_category ON flow_apps(category);

    CREATE TABLE IF NOT EXISTS app_executions (
      id TEXT PRIMARY KEY,
      app_id TEXT NOT NULL REFERENCES flow_apps(id),
      user_id TEXT,
      input_data TEXT,
      output_data TEXT,
      status TEXT,
    ...
    CREATE INDEX IF NOT EXISTS idx_app_executions_app_id ON app_executions(app_id);
    CREATE INDEX IF NOT EXISTS idx_app_executions_user_id ON app_executions(user_id);
...
// Flow App Store functions
export async function getApp(id) {
  const res = await pool.query("SELECT * FROM flow_apps WHERE id = $1", [id]);
  const row = res.rows[0];
  if (!row) return null;
  return {
    ...row,
...
export async function getAppBySlug(slug) {
  const res = await pool.query("SELECT * FROM flow_apps WHERE slug = $1", [slug]);
  const row = res.rows[0];
  if (!row) return null;
  return {
    ...row,
...
export async function listApps({ status = "published", limit = 50, offset = 0, category = null } = {}) {
  let query = "SELECT * FROM flow_apps WHERE status = $1";
  const params = [status];
  if (category) {
...
export async function createApp(data) {
  ...
  const res = await pool.query(
    `
      INSERT INTO flow_apps (id, flow_id, name, slug, description, icon_url, created_by, category, tags, status, input_schema, output_type, created_at, updated_at)
...
export async function updateApp(id, data) {
  ...
  const query = `UPDATE flow_apps SET ${updates.join(", ")} WHERE id = $${paramCount} RETURNING *`;
  const res = await pool.query(query, params);
  return res.rows[0];
}

export async function deleteApp(id) {
  await pool.query("DELETE FROM flow_apps WHERE id = $1", [id]);
}

export async function recordExecution(app_id, { user_id = null, input_data = null, output_data = null, status = "success", execution_time_ms = 0, error_message = null } = {}) {
  const id = crypto.randomBytes(16).toString("hex");
  const now = nowSec();

  await pool.query(
    `
      INSERT INTO app_executions (id, app_id, user_id, input_data, output_data, status, execution_time_ms, error_message, created_at)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
    `,
    [id, app_id, user_id, input_data ? JSON.stringify(input_data) : null, output_data ? JSON.stringify(output_data) : null, status, execution_time_ms, error_message, now]
  );

  // Increment usage count
  await pool.query("UPDATE flow_apps SET usage_count = usage_count + 1 WHERE id = $1", [app_id]);
}

export async function getExecutions(app_id, { limit = 20 } = {}) {
  const res = await pool.query(
    `
      SELECT * FROM app_executions WHERE app_id = $1 ORDER BY created_at DESC LIMIT $2
    `,
    [app_id, limit]
  );
  return res.rows.map((row) => ({
...
export async function addReview(app_id, user_id, rating, review_text = "") {
  const id = crypto.randomBytes(16).toString("hex");
  const now = nowSec();

  await pool.query(
    `
      INSERT INTO app_reviews (id, app_id, user_id, rating, review_text, created_at)
      VALUES ($1, $2, $3, $4, $5, $6)
    `,
    [id, app_id, user_id, rating, review_text, now]
  );

  // Update app rating
  const res = await pool.query(
    "SELECT AVG(rating)::DECIMAL as avg_rating FROM app_reviews WHERE app_id = $1",
    [app_id]
  );
  const avgRating = res.rows[0]?.avg_rating || 0;
  await pool.query("UPDATE flow_apps SET rating = $1 WHERE id = $2", [avgRating, app_id]);
}

export async function getReviews(app_id) {
  const res = await pool.query("SELECT * FROM app_reviews WHERE app_id = $1 ORDER BY created_at DESC", [app_id]);
  return res.rows;
}

export async function addFavorite(user_id, app_id) {
  const now = nowSec();
  await pool.query(
    `
      INSERT INTO user_app_favorites (user_id, app_id, created_at)
      VALUES ($1, $2, $3)
...
export async function removeFavorite(user_id, app_id) {
  await pool.query("DELETE FROM user_app_favorites WHERE user_id = $1 AND app_id = $2", [user_id, app_id]);
}

export async function getFavorites(user_id) {
  const res = await pool.query(
    `
      SELECT fa.*, fa2.* FROM user_app_favorites fa
      JOIN flow_apps fa2 ON fa.app_id = fa2.id
      WHERE fa.user_id = $1
