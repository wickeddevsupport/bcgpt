  ...
  return [
    ...
    ...(figmaConnected
      ? [
          ...
          "- If the user pastes a Figma file URL, anchor to that exact file, do not fall back to the selected FM file, and prefer live MCP discovery first: call `figma_mcp_list_tools`, then `figma_mcp_call` with the exact capability needed for comments, annotations, screenshots, structure, variables, or node context.",
          ...
          "- If official Figma MCP returns auth required, 405, or unavailable, then use `figma_pat_audit_file` to run a PAT-backed REST audit on the target file as fallback. Do not describe FM as offline when only official Figma MCP failed.",
...
function describeAssistantPolicySection(): string {
  return [
    ...
    "- NEVER ask the user to manually copy-paste data from Figma — use figma_get_context and web_fetch/web_search tools instead.",
